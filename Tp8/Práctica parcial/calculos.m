...
%ej5
%% ── MODIFICAR AQUÍ ──────────────────────────────────────────────────────────
H1 = K*(a*s + 1)*(b*s + 1) / s; %Controlador
H2 = 2*(s + 2) / ((s + 1)*(s + 10)); % Planta
...
%% Paso 6: TF del controlador y planta para Simulink

% Controlador: Gc(s) = (Gc_num) / s
fprintf('\n--- Transfer Function: Controlador ---\n')
fprintf('  Numerador:   [')
fprintf(' %.6f', Gc_num)
fprintf(' ]\n')
fprintf('  Denominador: [ 1 0 ]\n')

% Planta: H2(s) = 2*(s+2) / ((s+1)*(s+10))
H2_num = conv([2], [1 2]);              % 2*(s+2)     = [2 4]
H2_den = conv([1 1], [1 10]);           % (s+1)*(s+10) = [1 11 10]

fprintf('\n--- Transfer Function: Planta ---\n')
fprintf('  Numerador:   [')
fprintf(' %.6f', H2_num)
fprintf(' ]\n')
fprintf('  Denominador: [')
...
% Polos y ceros
fprintf('Polos controlador: '); disp(roots([1 0]).') 
fprintf('Ceros controlador: '); disp(roots(Gc_num).')
fprintf('Polos planta:      '); disp(roots(H2_den).')
fprintf('Ceros planta:      '); disp(roots(H2_num).')